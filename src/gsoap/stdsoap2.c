...
{ register int status = soap->error;
  ...
  //dongliqiang modified at 20130705 for TIME_WAIT
  if (soap_valid_socket(soap->socket))
  {
	  tcp_select(soap, soap->socket, SOAP_TCP_SELECT_RCV | SOAP_TCP_SELECT_ERR, 1);	//wait for 1S to colse socket
  }
  //add end
...
//dongliqiang added at 20120228
SOAP_FMAC1 void* SOAP_FMAC2 soap_mallocz(struct soap *soap, size_t n)
{
	void *p = NULL;
