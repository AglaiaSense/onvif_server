...
int RUDPSendV(RUDPSOCKET sock, int chno, const IOVEC *v, unsigned int size, int flags);


#define OPT_UDP_SNDBUF	1
#define OPT_UDP_RCVBUF	2
#define OPT_RUDP_SNDBUF	3
#define OPT_RUDP_RCVBUF	4
#define OPT_LINGER	5
#define OPT_FC		6
#define OPT_MSS		7
...
int RUDPSelectSock(RUDPSOCKET sock, int chno, /*INOUT*/int *r, const struct timeval *timeout);
...
#define RUDP_FD_SET(fd, prc, size) do { prc[size].sock = NULL; prc[size++].chno = fd; }while(0)
#define RUDP_SET(s, chn, prc, size) do { prc[size].sock = s; prc[size++].chno = chn; }while(0)
#define RUDP_CLR(s, chn, prc, size) do { int i; for(i=0; i<size; i++) \
