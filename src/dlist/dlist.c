...
bool X_API STDCALL adt_dlist_destroy(DList* dlist)
{
    if (dlist == NULL)
    {
...
DListNode* X_API STDCALL adt_dlist_add(DList* dlist, DListNode* node, void* data)
{
	...
	if(NULL == new_node)
	{
		perror("adt_dlist_add, malloc node failed");
		return NULL;
	}
	memcpy(new_node->data, data, dlist->node_size);
	new_node->next = new_node->prev = NULL;
...
bool X_API STDCALL adt_dlist_empty(DList* dlist)
{
	if (dlist == NULL)
	{
		td_printf(0,"dlist is null");
...
bool X_API STDCALL adt_dlist_delete(DList* dlist, DListNode* node)
{
    if (dlist == NULL)
	{
		td_printf(0,"dlist is null");
...
bool X_API STDCALL adt_dlist_lock_init(DList* dlist)
{
    if (dlist == NULL)
    {
		td_printf(0,"dlist is null");
...
bool X_API STDCALL adt_dlist_lock_uninit(DList* dlist)
{
    if (dlist == NULL)
    {
		td_printf(0,"dlist is null");
...
bool X_API STDCALL adt_dlist_lock(DList* dlist)
{
    if (dlist == NULL)
    {
		td_printf(0,"dlist is null");
	...
	//return os_mutex_lock(&dlist->mutex);
	return pthread_mutex_lock(&dlist->mutex.mutex);
}

bool X_API STDCALL adt_dlist_unlock(DList* dlist)
{
    if (dlist == NULL) {
		td_printf(0,"dlist is null");
		return TD_FALSE;
	...
	return pthread_mutex_unlock(&dlist->mutex.mutex);
}
